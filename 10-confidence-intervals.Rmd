...
<!-- 
Commenting out all of previous 10.1 - some of it got worked into new 9.5. May want to add in some of these examples into ch 9 in future
...
<!-- In general, we construct a confidence interval using what we know about an estimator's sampling distribution. Above, we used the multiplier 1.96 because we know that $\bar{x}$ follows a Normal distribution, and we wanted our "level of confidence" to be 95%. If we wanted to construct a 90% confidence interval, we would use 1.645 -->

<!-- $$\text{Estimate} \pm \text{Critical Value} * SE(\text{Estimate})$$ -->

<!-- A **critical value** is a cutoff value from either the $t$ or standard normal distribution that corresponds to the desired level of confidence for the interval (e.g. 90%, 95%, 99%).  -->
