...
In order to construct a confidence interval, we need to know the sampling distribution of a *standardized* statistic. Recall from Chapter \@ref(sampling) that we can standardize an estimate by subtracting its mean and dividing by its standard error:

$$\frac{Estimate - Mean(Estimate)}{SE(Estimate)}$$
Just like an estimator has a sampling distribution, the standardized form of an estimate also has a sampling distribution. Most standardized statistics follow a $N(0,1)$ or $t(df)$ distribution. Table \@ref(tab:samp-dist-table-ch-10) shows the distribution of the standardized statistics for many of the  common statistics we have seen previously. 
...
Note that many of these standardized estimators (all but the proportion and difference in proportions) involve $s$, the sample estimate of the standard deviation. If in fact the population standard deviation was known, we could replace the $s$'s in these formulas with $\sigma$'s, and the distribution would follow a $N(0,1)$ rather than a $t(df)$. When the population standard deviation is unknown - which is most likely to be the case in practice - we have to estimate it by $s$, which adds some uncertainty to our standardized statistic. Using the t-distribution (instead of $N(0,1)$) allows us to account for this uncertainty by allowing for more probability to fall in the tails of the distribution. 
...
We could compute 95% confidence intervals for all 10,000 of our repeated samples, and we would expect approximately 95% of them to contain the true mean; this is the definition of what it means to be "95% confident" in statistics. Another way to think about it, when constructing 95% confidence intervals, we expect that we'll only end up with an "unlucky" sample- that is, a sample whose confidence interval doesn't capture the true mean - just 5% of the time. 
...
As shown in Table \@ref(tab:ch10_samp_dist), the appropriate distribution for a confidence interval of $\hat{\pi}$ is $N(0,1)$, so we can use the critical value 1.96 to construct a 95% confidence interval.
...
This value represents the proportion of those that yawned after seeing a seed yawn (0.2941) minus the proportion of those that yawned with not seeing a seed (0.25). Using the $N(0,1)$ distribution, we can construct the following 95% confidence interval. 
