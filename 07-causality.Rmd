...
In this chapter we kick off the third segment of this book, statistical theory. Up until this point, we have focused only on exploratory data analysis and descriptive statistics. That is, we have summarized and explored relationships that exist *in our observed dataset*. Very often the data available to us is **observational data** – data that is collected via a survey in which nothing is manipulated or via a log of data (e.g., scraped from the web). As a result, any relationship we observe is limited to the sample of data, and the relationships are considered **associational**. In this chapter we introduce the idea of making inferences through a discussion of **causality** and **randomization**.
...
In a randomized experiment, we just showed that we can calculate the estimated causal effect ($b_1$) of a treatment using a simple regression model. 
...
In order to randomly assign each of these students to treatment or control, we can use the `complete_ra()` function in the `randomizr` package. The first argument `N` specifies the total number of cases to be randomized, which in this example is `r dim(ed_data)[1]`, the number of Kindergarteners in our dataset. The argument `num_arms` specifies the number of groups (i.e. treatment arms) you want to randomize into. In this case, we only want to randomize into 1 treatment and 1 control group, so `num_arms = 2`. 
...
# (i.e. it's dimensions).
# Here we set N to equal the number of rows
N <- dim(ed_data)[1] 
ed_data <- ed_data %>% 
  mutate(Trt_random = complete_ra(N = N))
```
...
In comparison, in an observational study, the two groups are not equivalent on these pre-treatment variables. In the same example above, let us imagine that instead of being randomly assigned to treatment, instead students with lower SES were assigned to the new specialized curriculum (T = 1), and those with higher SES were assigned to the business as usual curriculum (T = 0). To accomplish this in R, we can use the `arrange()` function to sort the dataset by `SES_CONT` and then assign the 167 students with the lowest SES (who are now in the first 167 rows) a value of 1 for the treatment group and the 168 students with the highest SES to a value of 0 for the control group. Note that the function `rep()` replicates a value (or vector of values) a specified number of times. 
...
```{r}
ed_data %>% 
  group_by(Trt_non_random) %>% 
  summarise_if(is.numeric, mean)
```
There are somewhat large differences between the treatment and control group on several pre-treatment variables. Notice that the two groups still appear to be balanced in terms of gender. This is because gender is in general not associated with SES. However, minority status and test scores are both correlated with SES, so assigning treatment based on SES (instead of via a random process) results in an imbalance on those other pre-treatment variables. Therefore, if we observed differences in test scores at the end of the year, it would be difficult to disambiguate whether the differences were caused by the intervention or due to some of these other pre-treatment differences. 
...
In our `ed_data` example above, let's assume that on average, all students will increase their test scores by about 20 points throughout the year, but those recieving the treatment increase their scores by an additional 10 points on average. That is, let's assume the treatment effect is $\beta_1 = 10$. Note, we would NOT know this in real life - this is what we are trying to estimate- but we can mimic this scenario in R for demonstration purposes.  

In most scenarios, the treatment will not raise everyone's score by exactly the same amount, nor will every student increase their scores by exactly the same amount over the course of the year. Students may increase their scores by 20 points *on average*, but there will likely be some random "noise" involved. That is, some students will increase their scores by slightly more than 20 and others by slightly less than 20, just by chance. We can use another one of R's random generating functions to mimic this random noise. 

Let's create a new variable `MATH_post_random` that corresponds to the (hypothetical) post-treatment math scores in the scenario where treatment was randomly assigned. For the students in the treatment group, we will add approximately 30 points on average to their pre-treatment math score using the function `rnorm(n = 1, mean = 30, sd = 1)`. The function `rnorm` generates a random variable from the normal distribution (which we will discuss in Chapter \@ref(sampling)) with a specified mean and standard deviation. For the students in the control group, we will add an average of 20 points to their pre-treatment math scores. Again, it is important to note this is for demonstration purposes only- in real life, we only observe the test scores, but we never observe the random process that generated them. 
```{r, echo = FALSE}
set.seed(73)
```
...
```{r}
ed_data <- ed_data %>% 
  mutate(MATH_post_random = 
           case_when(Trt_random == 1 ~ MATH + rnorm(1, 30, 1), 
                     Trt_random == 0 ~ MATH + rnorm(1, 20, 1)))
```

Here we used a new function `case_when()` inside the `mutate()` function. As its name suggests, `case_when()` mutates cases (i.e. rows) when they meet a specified criteria (e.g. `Trt_random == 1`) and assigns them a new value, which is specified after the tilde `~`. So, when students were part of the (randomly assigned) treatment group, we generated their post-test math score (`MATH_post_random`) by taking their pre-test score (`MATH`) and adding an average of 30 points. 

We can verify that this does result in the treatment group scoring approximately 10 points higher than the control group (since all students gain an average of 20 points during the year).

```{r}
ed_data %>% 
  group_by(Trt_random) %>% 
  summarise(post_avg = mean(MATH_post_random))
```
...
```{r}
ed_data <- ed_data %>% 
  mutate(MATH_post_non_random = 
           case_when(Trt_non_random == 1 ~ MATH + rnorm(1, 30, 1), 
                     Trt_non_random == 0 ~ MATH + rnorm(1, 20, 1)))
ed_data %>% 
  group_by(Trt_non_random) %>% 
  summarise(post = mean(MATH_post_non_random))
```
Note that in this case, even though the treatment did increase test scores by about 10 points on average (verify this by comparing the pre- and post-test average), the data makes it look like students in the treatment group only scored a couple points higher than those in the control group. This is because here treatment status is confounded with SES. Let's look at two regression models estimating the effect of treatment on math scores, with and without controlling for SES. 
```{r}
...
```
The two models give quite different indications of how effective the treatment is. In the first model, the estimate of the treatment effect is `r summary(fit1_non_random)$coefficients[2,1]`, but in the second model once we control for SES, the estimate is `r summary(fit2_non_random)$coefficients[2,1]`. 
