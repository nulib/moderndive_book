...
if(!file.exists("data/ed_data_rand_exp.csv")){
  ed_data_rand_exp <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vRZUqCh_ROL0eGZ97qszaThMLaL79g7AXi5xdLMILHHjs3iEVrKCv5TPapBXFE1w07BKMW8R7sVu71B/pub?gid=40509529&single=true&output=csv" %>%
    read_csv(na = "")
    write_csv(ed_data_rand_exp, "data/ed_data_rand_exp.csv")
} else {
  ed_data_rand_exp <- read_csv("data/ed_data_rand_exp.csv")
}
```

```{r, message = FALSE}
ed_data_rand_exp <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vRZUqCh_ROL0eGZ97qszaThMLaL79g7AXi5xdLMILHHjs3iEVrKCv5TPapBXFE1w07BKMW8R7sVu71B/pub?gid=40509529&single=true&output=csv")

glimpse(ed_data_rand_exp)
```
It includes information on 335 Kindergarten students: indicator variables for whether they are female or minority students, information on their parents' highest level of education, a continuous measure of the their socio-economic status (SES), and their reading and math scores. For our purposes, we will assume that - with the expection of `MATH_post` - these are all **pre-treatment variables** that are measured on students at the beginning of the year, before we conduct our (hypothetical) randomized experiment. `MATH_post` are math scores that are collected at the end of the school year, after students in the treatment group have received the new curriculum.
...
```{r eclsK-obs, echo=FALSE, message=FALSE}
if(!file.exists("data/ed_data_rand_exp.csv")){
  ed_data_obs <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vQr12mpR6bcQsVyKX9diAH8Ix7zq0cgy6QJEz7QVp_fFDK7hc6yZHWU0CuJWAXvSPoiWEAfavMHzwWx/pub?gid=2089779433&single=true&output=csv" %>%
    read_csv(na = "")
    write_csv(ed_data_obs, "data/ed_data_obs.csv")
} else {
  ed_data_obs <- read_csv("data/ed_data_obs.csv")
}
```

```{r, message = FALSE}
ed_data_obs <- read_csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vQr12mpR6bcQsVyKX9diAH8Ix7zq0cgy6QJEz7QVp_fFDK7hc6yZHWU0CuJWAXvSPoiWEAfavMHzwWx/pub?gid=2089779433&single=true&output=csv")
