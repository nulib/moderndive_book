...
The code for how we generated these random post-test scores in below, but don't worry about understanding exactly how it works. Just note that in both the `Trt_rand` and `Trt_non_rand` case, we are creating post-treatment math scores that increase a student's score by 10 points on average, if they received the new curriculum.
...
Let's also look the post-treatment test scores by group for the non-randomized experiment case. 
```{r}
ed_data %>% 
  group_by(Trt_non_rand) %>% 
...
```
Note that even though the treatment raised student scores by 10 points on average, in the observational case it looks like the treatment effect is *much* smaller. This is because treatment was confounded with SES and other pre-treatment variables. 
