...
Often times when running a randomized experiment in practice, you want to ensure that exactly half of your participants end up in the treatment group. In this case, you don't want to flip a coin for each participant, because just by chance, you could end up with 63% of people in the treatment group, for example. Instead, you can imagine each participant having an ID number, which is then randomly sorted or shuffled. You could then assign the first half of ID numbers to the treatment group, for example. R has many ways of mimicing this type of process as well. For example, in Section \@ref(ed-data), we will use the function `complete_ra` from the `randomizr` package to randomly assign half of our dataset to a treatment group and half to a control group. 
...
In order to randomly assign each of these students to treatment or control, we can use the `complete_ra()` function in the `randomizr` package. The first argument `N` specifies the total number of cases to be randomized, which in this example is `r dim(ed_data)[1]`, the number of Kindergarteners in our dataset. The default is to randomly assign into two equal size groups, which is what we want, so we do not have to specify anything else. 
...
N <- dim(ed_data)[1] 
ed_data_rand_exp <- ed_data %>% 
  mutate(T = complete_ra(N = N))
```

We can then take a look at the variable `T` in our new `ed_data_rand_exp` data and see that 167 students were assigned a value of 1, which we will define to be the treatment group and 168 were assigned a value of 0, which we will define to be the control group. 
...
```{r}
# Note the function `rep()` replicates a value a specified number of times 
ed_data_obs <- ed_data %>% 
  arrange(SES_CONT) %>%
  mutate(T = c(rep(1, 167), rep(0, 168)))
```
...
Let's create a new variable `MATH_post` that corresponds to the (hypothetical) post-treatment math scores in our randomized experiment. For the students in the treatment group, we will add approximately 30 points on average to their pre-treatment math score using the function `rnorm(n = 1, mean = 30, sd = 1)`. The function `rnorm` generates a random variable from the normal distribution (which we will discuss in Chapter \@ref(sampling)) with a specified mean and standard deviation. For the students in the control group, we will add an average of 20 points to their pre-treatment math scores. Again, it is important to note this is for demonstration purposes only- in real life, we only observe the test scores, but we never observe the random process that generated them. 
```{r, echo = FALSE}
set.seed(73)
```

```{r}
ed_data_rand_exp <- ed_data_rand_exp %>% 
  mutate(MATH_post = 
           case_when(T == 1 ~ MATH + rnorm(1, 30, 1), 
                     T == 0 ~ MATH + rnorm(1, 20, 1)))
```

Here we used a new function `case_when()` inside the `mutate()` function. As its name suggests, `case_when()` mutates cases (i.e. rows) when they meet a specified criteria (e.g. `T == 1`) and assigns them a new value, which is specified after the tilde `~`. So, when students were part of the (randomly assigned) treatment group, we generated their post-test math score (`MATH_post`) by taking their pre-test score (`MATH`) and adding an average of 30 points. 
...
ed_data_rand_exp %>% 
  group_by(T) %>% 
  summarise(post_avg = mean(MATH_post))
```
...
```{r}
fit <- lm(MATH_post ~ T, data = ed_data_rand_exp)
fit
```

Let's also create a set of (hypothetical) post-treatment test scores in our `ed_data_obs` data set as well for the non-randomized experiment case, and look at two regression models estimating the effect of treatment on math scores, with and without controlling for SES. 
```{r}
ed_data_obs <- ed_data_obs %>% 
  mutate(MATH_post = 
           case_when(T == 1 ~ MATH + rnorm(1, 30, 1), 
                     T == 0 ~ MATH + rnorm(1, 20, 1)))

fit1_obs <- lm(MATH_post ~ T, data = ed_data_obs)
summary(fit1_obs)$coefficients

fit2_obs <- lm(MATH_post ~ T + SES_CONT, data = ed_data_obs)
summary(fit2_obs)$coefficients
```

The two models give quite different indications of how effective the treatment is. In the first model, the estimate of the treatment effect is `r summary(fit1_obs)$coefficients[2,1]`, but in the second model once we control for SES, the estimate is `r summary(fit2_obs)$coefficients[2,1]`. This is because here treatment status is confounded with SES. 
...
```{r}
fit1_rand_exp <- lm(MATH_post ~ T, data = ed_data_rand_exp)
summary(fit1_rand_exp)$coefficients

fit2_rand_exp <- lm(MATH_post ~ T + SES_CONT, data = ed_data_rand_exp)
