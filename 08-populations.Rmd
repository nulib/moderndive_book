...
Perhaps we are interested in studying films from the 20th century. For example, say we want to answer the question: "Did the percentage of movies that pass the Bechdel Test increase throughout the 20th century?" The Bechdel Test is a measure of the representation of women in fiction. For a movie to pass the Bechdel Test it must meet three criteria:
...
Let's discuss how we might go about developing a sampling plan to tackle this question, using the terminology from Sections \@ref(terminology) and \@ref(sample_selection).

Our **population** of interest here is all movies released between the years 1900 and 1999. We must develop a **population frame** that includes a list of all movies in this population. Thankfully, IMDb has a fairly robust database of movies, and that data is available in a dataset called `movies` in the `ggplot2movies` package. Let's take a look at this data. It includes `r dim(movies)[2]` variables with information on `r dim(movies)[1]` movies and their ratings by users on IMDb. For now, we'll look at a smaller subset of just 4 of these variables: the title of the movie, the year it was released, its length in minutes, and its average rating on IMDb (out of 10 stars).
...
We can see that this dataset contains information on movies released from `r min(movies$year)` to `r max(movies$year)`. More information about this dataset can be found in this [documentation](https://cran.r-project.org/web/packages/ggplot2movies/ggplot2movies.pdf), which indicates that "movies were selected for inclusion if they had a known length and had been rated by at least one imdb user."
