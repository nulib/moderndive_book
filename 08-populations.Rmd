...
Table \@ref(tab:parameters-table) gives some common population parameters we might be interested in and their corresponding estimators that we use to calculate estimates from our sample data. Note that we have used many of these estimators in Parts I and II of the book when we calculated summary statistics to describe our data (e.g. mean, correlation, regression coefficients). The next few chapters will help establish the statistical theory and conditions under which we can use these estimators to infer things about their corresponding population parameters.

```{r parameters-table, echo=FALSE, message=FALSE}
if(!file.exists("rds/ch8_parameters.rds")){
  ch8_parameters <- "https://docs.google.com/spreadsheets/d/e/2PACX-1vR1SMKwojwYNKe51lrOut7xgnzvB1aZXsF9402EA02f2hnF98pvL6aY5cwkSvZmWojmpQmPrp48ubdY/pub?output=csv" %>% 
    read_csv(na = "")
...
ch8_parameters %>% 
  ...
  footnote(symbol = c("$s_y$ and $s_x$ are the sample standard deviations for $y$ and $x$ respectively"), escape = F)
```
