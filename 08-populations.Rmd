# Populations and Generalizability {#populations}
...
# setting skim() print options
# suppresses in-line graphics or spark graphics
...
## Populations & Sampling {#sample-selection}
...
Let's discuss how we might go about developing a sampling plan to tackle this question, using the terminology from Sections \@ref(terminology) and \@ref(sample-selection).
...
Table \@ref(tab:strata-probs-equal) shows the number of movies that were sampled per strata, `n_strata`. These numbers are proportional to the total number of movies in that decade in the population `N_strata`. This results in each movie in the population having equal probability of selection into the sample. 
```{r, eval = FALSE}
movies_strata_equal %>% 
  group_by(decade) %>% 
...
```{r strata-probs-equal, echo = FALSE}
movies_strata_equal %>% 
  group_by(decade) %>% 
  summarise(n_strata = n(), N_strata = unique(N_strata), 
            prob = n_strata / N_strata) %>% 
...
In Chapter \@ref(causality) we talked about the process of *random assignment* (e.g. in randomized experiments), and in this chapter we talked about the process of *random sampling*. These two types of randomization processes are at the core of inferential statistics: 
...
```{r RA-RS, echo=FALSE, fig.cap="Random Assignment vs. Random Sampling", out.width='110%', fig.align='center'}
...
So far we have just told you that random sampling allows you to make *generalizable* claims from your sample to a population. In Chapter \@ref(sampling) we will introduce and demonstrate the statistical theory for *why* this is true. 
