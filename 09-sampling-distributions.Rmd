...
The t-distribution depends on one parameter, $df$, which stands for "degrees of freedom". $df$ is a function of sample size; for example in many cases $df = n - 1$. In general, the t-distribution has the same shape as the Normal distribution (symmetric, unimodal), but it has **heavier tails**. As sample size increases (and therefore degrees of freedom increases), the t-distribution becomes a very close approximation to the Normal distribution. 

```{r, fig.cap="t-distribution", echo = FALSE, message=FALSE, warning=FALSE}
ggplot(data.frame(x = c(-6, 6)), aes(x = x)) +
    stat_function(fun = dnorm,
...
pF <- ggplot(data.frame(x = c(0, 30)), aes(x = x)) +
...
pChi <- ggplot(data.frame(x = c(0, 10)), aes(x = x)) +
