...
While it is not possible to travel back in time for this particular question, we can instead generate a way to study this exact question by creating a **simulation**. To do so, we begin by creating a situation in which we know the outcome (e.g., what team each person is cheering for) for every individual or observation in a population. As a result, we know what the population parameter value is (e.g. $\pi = 0.91$). Now we randomly select a sample of $n = 100$ observations and calculate the sample proportion (e.g. $\hat{\pi}=0.88$). And now we **repeat** this sampling process – each time selecting a different possible sample of $n = 100$ – many, many, many times (e.g., $10,000$ different samples of $n = 100$), each time calculating the sample proportion. At the end of this process, we now have many different estimates of the population proportion, each equally likely. 
...
kable(fan_data[1:10,])
```
Table \@ref(tab:fan_data_table) shows the simulation results for the first 10 samples, and Figure \@ref(fig:fan_data_2) shows a histogram of all 10,000 sample estimates. Later on in this chapter you will learn how to conduct this type of simulation yourself.
...
```{r fan_data_2, warning = FALSE, error = FALSE, message = FALSE}
...
```
Let's draw 10,000 repeated samples of n = 5 countries from this population. The data for the first 2 samples (replicates) is shown in \@ref(tab:s2_bias).
...
Table \@ref(tab:s2_bias_2) shows the results for the first 10 samples. If we look at the average of $s_n^2$ and $s^2$ across all 10,000 samples, we find that $s_n^2$ is biased; on average it is equal to `r mean(variances$s2_n)`. Remember that the true value of the variance in this population is $\sigma^2 = 57.5$. By dividing by $n – 1$ instead of $n$, however, the bias is removed. Therefore we use $s^2= \frac{\sum(x_i - \bar{x})^2}{(n-1)}$ as our usual estimator for $\sigma^2$ because it is unbiased. Notice that the sampling distribution of the sample variances shown in Figure \@ref(fig:s2_bias_3) is not Normal but rather is skewed right; in fact, it follows a chi-square distribution with $n-1$ degrees of freedom. 
```{r s2_bias_3}
mean(variances$s2_n)
mean(variances$s2)
