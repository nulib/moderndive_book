...
Let's randomly generate a population of size N = 1,000 from the standard normal distribution (i.e. $\mu = 0$ and $\sigma = 1$) using the function `rnorm`. Then let's take 10,000 repeated samples of size n = 5. The data for the first 2 samples (replicates) is shown in \@ref(tab:s2_bias). 
...
Table \@ref(tab:s2_bias) shows the results for the first 10 samples. If we look at the average of $s_n^2$ and $s^2$ across all 10,000 samples, we find that $s_n^2$ is biased; on average it is equal to `r mean(s2$s2_n)`. By dividing by $n – 1$ instead of $n$, the bias is removed. 

```{r s2_bias_2}
sd2 <- samples %>% group_by(replicate) %>% 
...
ggplot(sd2) + 
  geom_histogram(aes(x = sd2_biased), fill = "red", alpha = 0.8) +
  geom_histogram(aes(x = sd2_unbiased), fill = "green", alpha = 0.8) +
...
Let’s return to our age at the football game example. Let’s say you could estimate the average age of attendees by selecting a sample of $n = 10$ or by selecting a sample of $n = 100$. Which would be better? Why? A larger sample will certainly cost more – is this worth it? What about a sample of $n = 500$? Is that worth the it?
...
### Example using balls and sample distributions {#eg1} 
{shows histograms with sampling distributions with different sample sizes}
...
In our simulated data, we see that only 13% of our samples results in a estimated proportion that is $\pm 1\%$ of the true $\pi = 0.91$, but 92% of samples are within $\pm 5\%$. Across our 10,000 samples, the average sample proportion is `r mean(fan_data$prop_home_fans)`, which is a good approximation of the true $\pi = 0.91$. 
