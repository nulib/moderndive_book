...
While it is not possible to travel back in time for this particular question, we can instead generate a way to study this exact question by creating a **simulation**. To do so, we begin by creating a situation in which we know the outcome (e.g., what team they're cheering for) for every individual or observation in a population. As a result, we know what the population parameter value is (e.g. $\pi = 0.91$). Now we randomly select a sample of $n = 100$ observations and calculate the sample mean (e.g. $0.88$). And now we **repeat** this sampling process – each time selecting a different possible sample of $n = 100$ – many, many, many times (e.g., $10,000$ different samples of $n = 100$), each time calculating the sample proportion. At the end of this process, we now have many different estimates of the population proportion, each equally likely. Table XX shows the results for the first 10 samples, and Figure XX shows a histogram of these 10,000 sample estimates. 
...
We do this simulation in order to understand how close any one sample’s estimate is to the true population proportion. For example, it may be that we are usually within $\pm 1\%$? Or maybe $\pm 5\%$? We can ask further questions, like: on average, is the sample proportion the same as the population mean? In our simulated data, we see that only 13% of our samples results in a estimated proportion that is $\pm 1\%$ of the true $\pi = 0.91$, but 92% of samples are within $\pm 5\%$. Across our 10,000 samples, the average sample proportion is `r mean(fan_data$prop_home_fans)`, which is a good approximation of the true $\pi = 0.91$. 

```{r, warning = FALSE, error = FALSE, message = FALSE}
#calculate the proportion of times the sample proportion falls within 1% of the true population proportion
sum(fan_data$prop_home_fans < pi + 0.01 & fan_data$prop_home_fans > pi - 0.01) / I 
...
Of these, the most common and important is the **Normal Distribution**. As a result of the Central Limit Theorem, when sample sizes are large, most sampling distributions will be approximated well by a Normal Distribution. 
...
{Data example with $n = 5$, $s^2$ calculated with $n$, show simulated values and average value given}

The estimator that we use, $s^2$, is thus used because the original estimator, $s_n^2$, was biased. By dividing by $n – 1$ instead of $n$, the bias is removed. 
