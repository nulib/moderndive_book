...
Imagine that you want to know the proportion of individuals at a football game that are cheering for the home team, so you take a random sample of $n=100$ people and find that $88$ of them are fans of the home team. From this sample you calculate $\hat{\pi} = 88/100 = 0.88$, which is an *estimate* of the population proportion $\pi$. 
...
While it is not possible to travel back in time for this particular question, we can instead generate a way to study this exact question by creating a **simulation**. To do so, we begin by creating a situation in which we know the outcome (e.g., what team they're cheering for) for every individual or observation in a population. As a result, we know what the population parameter value is (e.g. $\pi = 0.91$). Now we randomly select a sample of $n = 100$ observations and calculate the sample proportion (e.g. $0.88$). And now we **repeat** this sampling process – each time selecting a different possible sample of $n = 100$ – many, many, many times (e.g., $10,000$ different samples of $n = 100$), each time calculating the sample proportion. At the end of this process, we now have many different estimates of the population proportion, each equally likely. Table \@ref(tab:fan_data_table) shows the results for the first 10 samples, and Figure XX shows a histogram of these 10,000 sample estimates. Later on in this chapter you will learn how to conduct this type of simulation yourself.

```{r fan_data_table, warning = FALSE, error = FALSE, message = FALSE, echo = FALSE}
set.seed(76)
I <- 10000 ### number of iterations (number of samples)
n <- 100 ##### sample size (number of individuals selected in a sample)
pi <- 0.91 ### true population proportion
fan_data <- data.frame(sample = seq(1:I),
                       num_home_fans = rbinom(I, n, pi)) %>% 
  mutate(prop_home_fans = num_home_fans / n)
kable(fan_data[1:10,])
```
...
#calculate the proportion of times the sample proportion falls within 1% of the true population proportion
sum(fan_data$prop_home_fans < pi + 0.02 & fan_data$prop_home_fans > pi - 0.02) / I 
sum(fan_data$prop_home_fans < pi + 0.05 & fan_data$prop_home_fans > pi - 0.05) / I 
mean(fan_data$prop_home_fans)
ggplot(fan_data) +
...
Once you have simulated many possible sample values across repeated samples, an important question is how you can summarize these. The way you summarize a sampling distribution is the same way you’d summarize any other data: 
...
Note that the mean of our simulated sampling distribution is `r mean(virtual_prop_red$prop_red)`, which is a good approximation to the true proportion of red balls in the ball ($\pi =$ `r sum(bowl$color == "red")/dim(bowl)[1]`). This is because the sample proportion $\hat{\pi} = \frac{# of "successes"}{# of trials}$ is an unbiased estimator of the population proportion $\pi$. 

```{r warning = FALSE, message = FALSE}
mean(virtual_prop_red$prop_red) #mean of sampling distribution of pi_hat
sum(bowl$color == "red")/dim(bowl)[1] #true population proportion
```
...
where $n$ is the number of observations. Until now, we have simply provided this to you as the estimator without reason. You might ask: why is this divided by $n – 1$ instead of simply by $n$? To see why, examine the following case.

Let's randomly generate a population of size N = 1,000 from the standard normal distribution (i.e. $\mu = 0$ and $\sigma = 1$) using the function `rnorm`. Then let's draw 10,000 repeated samples of size n = 5 from this population. The data for the first 2 samples (replicates) is shown in \@ref(tab:s2_bias). 

```{r s2_bias}
set.seed(76)
population <- data.frame(x = rnorm(n = 1000, mean = 0, sd = 1))
samples <- rep_sample_n(population, size = 5, reps = 10000)
kable(samples[1:10,])
```
...
Table \@ref(tab:s2_bias) shows the results for the first 10 samples. If we look at the average of $s_n^2$ and $s^2$ across all 10,000 samples, we find that $s_n^2$ is biased; on average it is equal to `r mean(s2$s2_n)`. Remember that the true value of the variance in this population is $\sigma^2 = 1$. By dividing by $n – 1$ instead of $n$, however, the bias is removed. Therefore we use $s^2= \frac{\sum(x_i - \bar{x})^2}{(n-1)}$ as our usual estimator for $\sigma^2$ because it is unbiased. Notice that the sampling distribution of the sample variances shown in Figure \@ref(fig:s2_bias_2) is not Normal but rather is skewed right; in fact, it follows a chi-square distribution with $n-1$ degrees of freedom. 

```{r s2_bias_2}
variances <- samples %>% group_by(replicate) %>% 
  summarise(s2_n = sum((x - mean(x))^2) / 5, #calculate the variance for each sample with n = 5 on the denominator
            s2 = sum((x - mean(x))^2) / 4) #calculate the variance for each sample with n - 1 = 4 on the denominator
kable(variances[1:10,])
```
```{r s2_bias_3}
mean(variances$s2_n)
mean(variances$s2)
...
ggplot(variances) + 
  geom_histogram(aes(x = s2_n, fill = "red"), color = "black", alpha = 0.6) +
  geom_histogram(aes(x = s2, fill = "green"), color = "black", alpha = 0.6) +
...
{shows $n = 5$, $n = 30$, $n = 100$ for underlying data that is skewed}
```{r}
...
infant_mortality_noNA <- filter(gapminder, !is.na(infant_mortality))
sample_5 <- rep_sample_n(infant_mortality_noNA, size = 5, reps = 1000) %>% group_by(replicate) %>% 
  summarise(mean_infant_mortality = mean(infant_mortality)) %>% 
  mutate(n = 5)
sample_30 <- rep_sample_n(infant_mortality_noNA, size = 30, reps = 1000) %>% group_by(replicate) %>% 
  summarise(mean_infant_mortality = mean(infant_mortality)) %>% 
  mutate(n = 30)
sample_100 <- rep_sample_n(infant_mortality_noNA, size = 100, reps = 1000) %>% group_by(replicate) %>% 
  summarise(mean_infant_mortality = mean(infant_mortality)) %>% 
  mutate(n = 100)
...
Overall, this simulation shows that not only might the precision of an estimate differ as a result of a larger sample size, but also the sampling distribution might be different for a smaller sample size (e.g., $n = 10$) than for a larger sample size (e.g., $n=100$). 

In fact, in many cases, when the sample size is large, the sampling distribution is very close to a normal distribution. This is the Central Limit Theorem. 
