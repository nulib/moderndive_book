...
3.	What is its standard deviation of this distribution? What range of values are likely to occur in samples of size n?
...
The t-distribution depends on one parameter, $df$, which stands for "degrees of freedom". Degrees of freedom are a function of sample size; for example in many cases $df = n - 1$. In general, the t-distribution has the same shape as the Normal distribution (symmetric, unimodal), but it has **heavier tails**. As sample sizes increase (and therefore degrees of freedom increase), the t-distribution becomes a very close approximation to the Normal distribution. 
...
ggplot(data.frame(x = c(-6, 6)), aes(x = x)) +
  ...
  scale_linetype_manual(values = c(1, 3, 2)) +
  labs(title = "t-distribution", ylab = "") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))
```

The Chi-squared distribution and F-distribution are both unimodal but skewed right. The Chi-squared distribution depends on one parameter $(df)$, and the F-distribution depends on two parameters $df_1$ and $df_2$. 
```{r, echo = FALSE, message=FALSE, warning=FALSE}
pF <- ggplot(data.frame(x = c(0, 50)), aes(x = x)) +
...
pChi <- ggplot(data.frame(x = c(0, 6)), aes(x = x)) +
...
grid.arrange(pF, pChi, ncol = 2)
```

Of these four common distributions, the most common and important is the **Normal Distribution**. As a result of the Central Limit Theorem (CLT), when sample sizes are large, most sampling distributions will be approximated well by a Normal Distribution. In fact, we can see in Figure \@ref(fig:samplingdistribution-virtual-10000) that the sampling distribution of $\hat{\pi}$ follows a Normal distribution. We will discuss the CLT further in Section \@ref(clt)
...
*	A *large* standard error means that an estimate (e.g., in the sample you have) may be far from the population parameter. This means the estimate is *imprecise*. 
*	A *small* standard error means an estimate (e.g., in the sample you have) is likely to be close to the population parameter. This means the estimate is *precise*. 
...
1. Compute the resulting 10,000 replicated of the proportion of the shovel's balls that are red.
