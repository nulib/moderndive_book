...
1. The `moderndive` package, which accompanies this book and includes data sets we will analyze
...
Furthermore, let's look at a random sample of five out of the `r nrow(gapminder2007)` countries in Table \@ref(tab:model2-data-preview). Again, note due to the random nature of the sampling, you will likely end up with a different subset of 5 rows.
...
```{r lifeExp2007hist, echo=TRUE, warning=FALSE, fig.cap="Histogram of Life Expectancy in 2007."}
...
```{r catxplot0b, echo=FALSE, warning=FALSE, fig.cap="Life expectancy in 2007."}
...
Let's put these values in Table \@ref(tab:continent-mean-life-expectancies), which we'll revisit later on in this section.
...
```{r continent-mean-life-expectancies-2, echo=FALSE}
gapminder2007 %>%
  group_by(continent) %>%
  summarize(mean = mean(lifeExp)) %>%
