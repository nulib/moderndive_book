...
* Confidence intervals in Chapter \@ref(CIs) (`lower_ci` and `upper_ci`)
* Hypothesis testing in Chapter \@ref(hypothesis-tests) (`statistic` and `p_value`). 
...
not only the 21st instructor but for all `r nrow(evals_ch5)` instructors in the study? Recall that each instructor corresponds to one of the `r nrow(evals_ch5)` rows in the `evals_ch5` data frame and also one of the `r nrow(evals_ch5)` points in the regression plot in Figure \@ref(fig:numxplot4). 
...
Now let's say we repeated this process for all `r nrow(evals_ch5)` instructors in our
dataset. Regression *minimizes the sum of all `r nrow(evals_ch5)` arrow lengths
